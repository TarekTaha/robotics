	...
	while (openList->Start != NULL)
	{
    	...
    	if (goalReached(current) && current!= root)                     
		{
			...
			printf("\nLast Node destination: %f %f",current->pose.p.x(),current->pose.p.y());
      		current->next = NULL;
    		printf("\n	--->>> Goal state reached with :%d nodes created and :%d nodes expanded  Depth is:%d<<<---",ID,NodesExpanded,current->depth);
			printf("\n	--->>> General Clean UP <<<---");
			fflush(stdout);
			int m=0;
			Node * p;
			...
			p = current;
   			while (p != NULL)
			{
				// unlink the nodes from the closedList before freeing it
				closedList->unlink(p);
				openList->unlink(p);
				toBeFreed->unlink(p);
...
//		openList->unlink(current);
//		if(!toBeFreed->has(current))
//			toBeFreed->add(current);
		// put the current node onto the closed list, ==>> already visited List
//    	cout<<"\n Adding to closed List"; fflush(stdout);
//		closedList->add(current);
//		  			Node * tarek =closedList->Start;
//		  			while(tarek)
//		  			{
//		  				cout<<"\n 1-Tarek p X:"<<tarek->pose.p.x()<<" Y:"<<tarek->pose.p.y();
//		  				tarek=tarek->next;
//		  			}  				
//		cout<<"\n Added to closed List"; fflush(stdout);
		...
		{
			printf("\n	--->>> Search Ended On this Branch / We Reached a DEAD END Depth:=%d",current->depth);
		}
		else 
		while (childList != NULL)               
		{
//			cout<<"\n ChildList"; fflush(stdout);
  		    curChild  = childList;
  			childList = childList->next;
//  			Node * tarek =closedList->Start;
//  			while(tarek)
//  			{
//  				cout<<"\n Tarek p X:"<<tarek->pose.p.x()<<" Y:"<<tarek->pose.p.y();
//  				tarek=tarek->next;
//  			}  			
			Node * p;
			// test whether the child is in the closed list (already been there)
			if((p = closedList->has(curChild)))
			{
				if((curChild->f_value >= p->f_value) && (p->direction == curChild->direction))
					continue;
				else
				{
//					if(!toBeFreed->has(p))
